    ...
    fn handle(&mut self, loc: u64, data: std::io::Result<&[u8]>) {
        match data {
            Ok(data) => {
                ...
                if let Err(e) = &node {
                    let _ = self.nodes.insert_error(loc as u32, e.clone());
                    return;
                }

                let node = node.unwrap();
                if node.get_header().block != loc {
                    let _ = self.nodes.insert_error(loc as u32, NodeError::BlockNrMismatch);
                    return;
                }
...
fn read_internal_nodes(
    ...
    for _d in (0..depth).rev() {
        let mut handler = LayerHandler::new(is_root, aggregator, ignore_non_fatal, nodes);
        is_root = false;
...
fn collect_nodes_in_use(
    ...
    for root in roots {
        let _ = read_internal_nodes(ctx, &mut pool, aggregator, *root as u32, ignore_non_fatal, &mut nodes);
    }
    ...
    let monitor = ProgressMonitor::new(
        ...
        move || {
            mon_meta_sm.lock().unwrap().get_nr_allocated().unwrap()
                + (mon_data_sm.get_nr_allocated().unwrap() / 8)
        },
    );
