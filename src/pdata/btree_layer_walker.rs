        ...
        match data {
            Ok(data) => {
                match &check_and_unpack_node_::<u64>(data, loc, self.ignore_non_fatal, self.is_root)
                {
                    ...
                    Ok(_) => {
                        self.last_err = Some(anyhow!("node {} is not an internal", loc));
                    }
                    Err(e) => {
                        self.last_err = Some(anyhow!("{} at block {}", e, loc));
                    }
                }
            }
            Err(e) => {
...
pub fn read_internal_nodes<V: Unpack>(
    ...
    for _d in (0..depth).rev() {
        ...
        if let Some(e) = handler.last_err {
            return Err(e);
        }
        ...
        match data {
            Ok(data) => {
                match &check_and_unpack_node_(data, loc, self.ignore_non_fatal, self.is_root) {
                    ...
                    Ok(_) => {
                        self.last_err = Some(anyhow!("node {} is not a leaf", loc));
                    }
                    Err(e) => {
                        self.last_err = Some(anyhow!("{} at block {}", e, loc));
                    }
